...
class Pipe:
    def __init__(self, x, y, speed):
        ...
        self.bot_rect = Rect(x, y + self.height + 200, self.width, HEIGHT - self.height - 100)
        
    def update(self):
        global score
        self.top_rect.x -= self.speed
        self.bot_rect.x -= self.speed
    ...
    def draw_pipe(self):
        ...
        draw.rect(screen, white, self.bot_rect)
        
    def reset(self):
        self.height = randint(100,300)
        self.top_rect = Rect(WIDTH, 0, self.width, self.height)
        self.bot_rect = Rect(WIDTH, self.height + 200, self.width, HEIGHT - self.height - 100)
...
background = transform.scale(image.load("Pygame/FlappyMiner/background.jpg"), (WIDTH, HEIGHT))

miner = Miner("Pygame/FlappyMiner/miner.png", 100, 300, 10)
...
while run:
    ...
    for pipe in pipes:
        pipe.update()
        pipe.draw_pipe()
        if miner.rect.colliderect(pipe.top_rect) or miner.rect.colliderect(pipe.bot_rect):
            print("Game over!")
            run = False
